...
public class Main {
    ...
    private static Type inputTaskType(Scanner scanner) {
        ...
        switch (typeChoice) {
            case 1 -> type = Type.PERSONAL_TASK;
            case 2 -> type = Type.WORK_TASK;
            default -> {
    ...
    private static void createTask(String title, String description, Type type, LocalDateTime taskTime, int appearsIn) {
        ...
        try {
            switch (appearsIn) {
                case 1 -> task = new OneTimeTask(title, description, taskTime, type);
                case 2 -> task = new DailyTask(title, description, taskTime, type);
                case 3 -> task = new WeeklyTask(title, description, taskTime, type);
                case 4 -> task = new MonthlyTask(title, description, taskTime, type);
                case 5 -> task = new YearlyTask(title, description, taskTime, type);
